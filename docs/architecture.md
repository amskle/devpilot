...
Phase 7 在稳定门面中增加 Replay 与 Evaluation：Event Store 用于事件完整性重放，LangGraph Checkpoint 用于真实 State Replay，RecoveryPoint Fork 创建隔离任务；评测报告持久化 Dataset 快照、Dataset/Prompt 摘要和目标 Task 引用。
...
Replay 与评测契约见 [phase7-replay-evaluation.md](phase7-replay-evaluation.md)。
