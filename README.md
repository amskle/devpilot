...
预算策略 v2 的默认总 Token 上限为 `200000`、模型调用上限为 `20`。Planning 单节点最多使用 `80000` Token 和 6 次模型生成，并为最后一次无工具 JSON 响应预留空间；单次生成的输入按实际请求动态估算，输出（含推理 Token）硬限制为 `4096`。大型任务也可在创建时追加 `--max-total-tokens 250000 --max-llm-calls 30`；通过 `task create --help` 可查看其他预算参数。已创建的旧任务保留原上限并标记为策略 v1，已使用量始终由 DevPilot 维护。
